    ...
    if (bit_istrue(settings.status_report_mask,BITFLAG_RT_STATUS_SERIAL_RX)) {
      ...
      if (bit_istrue(settings.status_report_mask,
        ...
        if (bit_istrue(settings.status_report_mask,BITFLAG_RT_STATUS_PROBE_PIN)) {
		  #ifdef CARVIN // this is here to make it report like it did in 1.1.5.  Eventually we should change to 1=true for all pins
		  if (probe_get_state()) { printPgmString(PSTR("0")); }
          else { printPgmString(PSTR("1")); }
		  #else
